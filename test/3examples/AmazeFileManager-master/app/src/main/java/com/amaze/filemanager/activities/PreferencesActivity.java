...
public class PreferencesActivity extends ThemedActivity {
    ...
    //@cmt
    //ActivityLifeRecorder  活动的生命周期进文件持久化
    static class ActivityLifeRecorder {

        static Queue<String> queue = null;

        static File output_file = null;

        static void Init_ActivityLifeRecorder(){
            if(queue == null) {
                queue = new LinkedList<String>();
            }
            //打开记录文件,没有则新建该文件
            output_file =  new File("/storage/sdcard0/AmazeFileManger_Preferences.txt");
        ...
        public static void record_time(String funtion_name){

            queue.offer( funtion_name+"  "+System.currentTimeMillis() );
        }

        public  static void write_time_to_file(){
            try {
                BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(output_file, true)));
                out.write("\n\n");
                String str;
    ...
    public void onCreate(Bundle savedInstanceState) {
        //@cmt
        ActivityLifeRecorder.Init_ActivityLifeRecorder();
        ActivityLifeRecorder.record_time("onCreate start");


        super.onCreate(savedInstanceState);
        setContentView(R.layout.prefsfrag);
        getWindow().setBackgroundDrawable(null);//@cmt
        ...
        //@cmt
        ActivityLifeRecorder.record_time("onCreate end");
        ActivityLifeRecorder.write_time_to_file();
    }

    //@cmt
    @Override
    protected void onRestart(){
        ActivityLifeRecorder.record_time("onRestart start");

        super.onRestart();

        ActivityLifeRecorder.record_time("onRestart end");
        ActivityLifeRecorder.write_time_to_file();
    ...
    public void onResume() {
        ActivityLifeRecorder.record_time("onResume start");

        super.onResume();
        //@cmt
        ActivityLifeRecorder.record_time("onResume end");
        ActivityLifeRecorder.write_time_to_file();
    }
    ...
    protected void onStart() {
        ...
        ActivityLifeRecorder.record_time("onStart start");

        super.onStart();

        ActivityLifeRecorder.record_time("onStart end");
        ActivityLifeRecorder.write_time_to_file();
    }
